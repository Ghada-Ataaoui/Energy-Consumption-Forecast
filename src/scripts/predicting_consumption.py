...
data = pd.read_csv('fridge_data_for_prediction.csv')
# print(data.info())
# print(data.head())
...
# Recalculate the timestamps for the validation set predictions
timestamps = data.index[train_size + time_steps:train_size + time_steps + len(predictions)]

# Visualization with timestamps on x-axis
plt.figure(figsize=(10, 7))
plt.plot(timestamps, y_val_org[:, 0], label="Actual Consumption")
plt.plot(timestamps, predictions[:, 0], label="Predicted Consumption")
plt.xlabel('Time')
